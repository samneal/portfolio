...
public class redX : MonoBehaviour {
	...
	void Start () {
		iTween.MoveTo(gameObject,iTween.Hash("path",iTweenPath.GetPath("redXPath"),"time", spawnRed.redSpeed, "looptype", iTween.LoopType.none, "easetype", iTween.EaseType.linear, "movetopath", false));
	...
	void OnCollisionEnter(Collision cc){
		...
		if (cc.gameObject.tag == "blueMinion") {
			enemyAttack = enemyMinion.GetComponent<blueX> ().blueMinAttack;
